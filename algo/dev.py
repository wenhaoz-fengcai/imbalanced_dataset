...
class DEVALGO(AdaBoostClassifier):
    ...
    def _boost_real(self, iboost, X, y, sample_weight, random_state):
        ...
        # Only counts the misclassification on original data
        minx = [ind for ind, exam in enumerate(X) if y[ind] == self.minority_target]
        mask_array = np.zeros(X.shape[0], dtype=int)
        mask_array[minx] = 1
        incorrect = mask_array * incorrect
